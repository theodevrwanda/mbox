...
<head>
    ...
    <title>Mbox | more <?php echo $morekey;?> movies </title>
...
while($recent = $recentm->fetch_assoc()){
 ...
 echo '<img src="'.$recent['poster_link'].'">';
 echo '</div>';
 echo '<div class="product-name">'.$recent['name'].'</div>';
 echo '<div class="product-price">'.$recent['translator'].'</div>';
 echo ' </article>';
 ...
 <section class="movies">
  <!-- populary  -->
    <div class="movie-category">
            ...
            <div class="head">
                <h1><?php echo $morekey;?></h1>
            </div>
   
            <div class="items">
            <?php
    
    $moviesall = "SELECT * FROM movies WHERE category = '$morekey' ORDER BY id desc";
    ...
    while ($moviesallR = $actionMoviesResult->fetch_assoc()) 
       {    
            ...
            echo '<h2 class="title">'.$moviesallR['name'].'<br><small>'.$moviesallR['translator'].'</small></h2>';
            echo '<p class="copy">'.$moviesallR['description'].'</p>';
       ...
       for ($i;$i<=$counter;$i++) 
       {    
            echo ' <div class="item" style="background-repeat: no-repeat;background-size: cover;background-position: center;">';
            echo '<div class="content">';
            echo '<h2 class="title"><small>Content Loading..</small></h2>';
            echo '<p class="copy"></p>';
            echo '<a class="button" style="vertical-align:middle; text-decoration:none;"  target="_blank"><button style="background: transparent; border:none; color:white;"><span>wait..</span></button></a>';
            echo ' </div> </div>  ';
       }
    ?>         
  </div>
...
<!-- feedback input -->
 <section class="feed">
       <form action="../feed.php" method="post" class="feedback">
        ...
        <button type="submit" name="sendfrommore">Send</button>
      </form>
 </section>
