...
if (isset($_POST['query'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM movies WHERE name LIKE CONCAT('%', ?, '%')");
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<h3><a href="views.php?id=' . $row['id'] . '" target="_blank">' . $row['name'] . '</a></h3>';            
...
<head>
  ...
  <title>Mbox | Watch Movies</title>
  <link rel="stylesheet" href="../issets/styles/views.css">
    ...
    <header>
        ...
        <nav>
            <ul>
                ...
                <li><a href="allmovies.php"  >All Movies<span></span></a></li>
            </ul>
        </nav>
<!-- Search form -->
...
<section id="searchResults" class="result"></section>
    ...
    if(empty($openkey))
{
  ...
  $moviequery = "SELECT * FROM movies WHERE id = $openkey";
  $movieresult = $conn->query($moviequery);
  $final = $movieresult->fetch_assoc();
  $typekey = $final['category'];
?>

<iframe width="560" height="315" 
src="<?php echo htmlspecialchars($final['trail_url'],); ?>"

    frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
</iframe>
      <div class="movie-details">
        <h2><?php echo $final['name'].'<br>'.'<small>'. $final['translator'].'</small>';?></h2>
        ...
        <div class="action-buttons">
           <a href="mylist.php?id=<?php echo $final['id'];?>"><button class="watch-button">Add To List</button></a>
           <a href="<?php echo $final['download_link'];?>" target="_blank"><button class="download-button">Watch Or Download</button></a>  
        </div>
      </div>

    </div>
    
    <!-- Sidebar with More Films -->
    <div class="foryou">
       <h3>More Films</h3>
        <div class="film-list">

<?php
$relationkey = $conn->real_escape_string($final['relationship']);
$relationquery = "SELECT * FROM movies WHERE relationship = '$relationkey' ";
$relationfinal = $conn->query($relationquery);
if ($relationfinal) {
    while ($relation = $relationfinal->fetch_assoc()) {
        echo '<div class="film-item">';
        echo '<img src="' . $relation['poster_link'] . '" alt="Film Thumbnail">'; // Dynamic poster image from DB
        echo '<div class="film-details">';
        echo '<h4><a href="views.php?id=' . $relation['id'] . '" style="text-decoration: none;color: white;">' . $relation['name'] . '</a></h4>'; // Dynamic movie name
        echo '<strong>' . $relation['title'] . '</strong><br>'; // Dynamic movie title or part
        echo '<small>' . date("F d, Y", strtotime($relation['data_uploaded'])) . '</small>'; // Dynamic upload date formatted
        echo '</div>';
        echo '</div>';
    }
} else {
...
<!-- other movies have simalar to this movies you want to download or watch 0 -->
<main>
...
$populary = "SELECT * FROM movies WHERE category = '$typekey' ORDER BY id desc LIMIT 24";
...
while ($popularyR = $actionMoviesResult->fetch_assoc()) 
{
 ...
 echo '<h2 class="title">'.$popularyR['name'].'<br><small>'.$popularyR['translator'].'</small></h2>';
 echo '<p class="copy">'.$popularyR['description'].'</p>';
 echo '<a class="button" href="views.php?id='.$popularyR['id'].'" style="vertical-align:middle; text-decoration:none;"><button style="background: transparent; border:none; color:white;"><span>Download </span></button></a>';
 echo ' </div> </div>  ';
}
?>         
</div>
...
<section class="feed">
   <form action="../feed.php" method="post" class="feedback">
    <input type="hidden" value="<?php echo $openkey ;?>" name="key"> 
    <textarea name="feedback" placeholder="write your feedback here"></textarea>
    <button type="submit" name="sendfromviews">Send</button>
  </form>
</section>
 ...
 <footer>
  <div class="footer-container">
      ...
      <div class="footer-links">
          ...
          <ul><li><a href="../home.php">Back to Home</a></li>
          <?php
